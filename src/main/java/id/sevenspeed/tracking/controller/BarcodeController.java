...
public class BarcodeController {
    ...
    public ResponseEntity<ApiResponse<List<BarcodeResponse>>> findByBatchId(
        ...
        return ResponseEntity.ok(ApiResponse.ok(
                barcodeService.findByBatchId(id).stream()
    ...
    public ResponseEntity<ApiResponse<BarcodeResponse>> generate(
        ...
        Barcode barcode = barcodeService.generate(id, request);
        return ResponseEntity.status(201).body(ApiResponse.ok(BarcodeResponse.from(barcode)));
    }
    ...
    public ResponseEntity<ApiResponse<BarcodeResponse>> update(
        ...
        Barcode barcode = barcodeService.update(id, request);
        return ResponseEntity.ok(ApiResponse.ok(BarcodeResponse.from(barcode)));
    }
}